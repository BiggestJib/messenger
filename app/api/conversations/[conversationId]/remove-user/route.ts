...
// API route handler to remove a user from an existing group conversation
export async function POST(
  ...
  try {
    ...
    const { userId } = body; // Extract the user ID to be removed from the request body
    ...
    const userInConversation = conversation.users.some(
      (user) => user.id === userId // Check if the user's ID exists in the conversation
    );

    if (!userInConversation) {
      return new NextResponse("User is not a member of this conversation", {
        status: 400,
      }); // Return a 400 Bad Request response if the user is not in the group
    }
    ...
    const updatedConversation = await prisma.conversation.update({
      ...
      data: {
        users: {
          disconnect: { id: userId }, // Remove the user from the users array
        },
      },
      include: { users: true }, // Include the updated list of users in the result
    });

    // Return the updated conversation with the user removed
    return NextResponse.json(updatedConversation);
  } catch (error: any) {
    // Catch any errors that occur and return a 500 Internal Server Error response
    console.error("Error removing user from conversation:", error); // Log the error for debugging purposes
